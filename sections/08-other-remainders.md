...
<a id="fn-6"></a>**6.**&nbsp; One incidental finding while checking [(61)](#eq-siegel-f) against the source: the translation’s equation 83 prints the sum as $`\sum n^{s-1}`$, while equation 84, derived from it, has $`\sum n^{-s}`$. The numerics say $`n^{-s}`$ is correct, so equation 83 carries a typo in the Barkan–Sklar translation. [↩](#fnref-6)
