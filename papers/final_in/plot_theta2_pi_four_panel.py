#!/usr/bin/env python3
"""
Four skinny fold-angle panels, left → right:
  4.65–4.80 | 9.42–9.46 | 17.2–17.6 | 0–20
Same layout and windows as plot_leg_equality_four_panel.py, but the field is
the mean fold deviation tau-bar: how far theta_2 is from pi, averaged over
the ps, R/2 and ak splits (dark blue = all three fold back).
Zeros @ σ=1/2 (green); zoom bands linked to the full strip by connection
lines.
"""
...
ZEST = HERE.parents[2]
OUT_PNG = HERE / "figures" / "theta2_pi_four_panel.png"
OUT_PDF = HERE / "figures" / "theta2_pi_four_panel.pdf"
...
PANELS = [
    ("theta2_pi_strip_4p65_4p80", r"$4.65 \leq T \leq 4.80$"),
    ("theta2_pi_strip_9p42_9p46", r"$9.42 \leq T \leq 9.46$"),
    ("theta2_pi_strip_17p2_17p6", r"$17.2 \leq T \leq 17.6$"),
    ("theta2_pi_strip_0_20", r"$0 \leq T \leq 20$"),
]
...
def main() -> None:
    ...
    fig.suptitle(
        r"Mean fold deviation $\bar\tau$  (dark blue = $\vartheta_2=\pi$ for"
        r" all three splits)"
        "\n"
    ...
    cbar.set_label(r"mean $\tau$ (log; dark = folded back)", fontsize=8)
