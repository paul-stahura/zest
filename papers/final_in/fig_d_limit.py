...
ax.plot(xs, d(xs), 'b-', lw=1.8, label=r'$d(x)$ (closed form)', zorder=3)

for N, c, mk in ((10, '#cc7722', 'x'), (50, '#22aa55', '+'), (400, 'r', '.')):
    grid = np.arange(0.02, 1.0, 0.02)
    vals = [d1hat(mp.mpf(N)+mp.mpf(f'{x:.3f}')) for x in grid]
    ax.plot(grid, vals, mk, color=c, ms=5, label=fr'$\sqrt{{m+1}}\,d_1$ at $T={N}+x$')
...
ax.text(0.40, 1-m0, r'$1-m_0=0.7731048$', va='bottom', ha='center', color='r', fontsize=9)
...
ax.set_title(r'The limit profile $d(x)=\lim_{T\to\infty,\ \{T\}=x}\sqrt{m+1}\,d_1(T)$ on $\sigma=\frac{1}{2}$')
ax.grid(alpha=0.3); ax.legend(loc='upper left', fontsize=9)
fig.tight_layout()
fig.savefig('fig_d_limit.png', dpi=160)
print("saved fig_d_limit.png")
