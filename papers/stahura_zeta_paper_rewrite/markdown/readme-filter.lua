...
-- A footnote's continuation lines have to stay indented to belong to the
-- footnote, so a display fence inside one is always indented and hits the
-- same GitHub bug. There is nowhere to move it to, so the equation is
-- rendered as an inline span instead -- footnote text is small anyway.
--
-- Pandoc walks inlines before blocks, so this runs before the Para splitter
-- above and the display math never reaches it.
function Note(el)
  el.content = el.content:walk({
    Math = function(m)
      if m.mathtype == 'DisplayMath' then
        return pandoc.Math('InlineMath', m.text)
      end
    end,
  })
  return el
...
function Table(el)
  ...
  -- The anchor is emitted as raw HTML, so clear the identifier; otherwise
  -- the gfm writer appends it to the caption as a literal "{#tab:frac}".
  el.identifier = ''

  local cap = el.caption
  if cap and cap.long and #cap.long > 0 then
    local inlines = pandoc.utils.blocks_to_inlines(cap.long)
    local head = pandoc.List()
    head:insert(pandoc.Strong({ pandoc.Str('Table ' .. tab_count .. ':') }))
    head:insert(pandoc.Space())
    head:extend(convert_inlines(inlines, false))
    el.caption = pandoc.Caption({ pandoc.Plain(head) })
  end
...
function CodeBlock(el)
  ...
  -- Assigning a bare table to el.classes does not take; rebuild the Attr.
  if #el.classes == 0
     and (el.text:match('theorem') or el.text:match('lemma'))
     and el.text:match(':=') then
    return pandoc.CodeBlock(el.text, pandoc.Attr(el.identifier, { 'lean' },
                                                 el.attributes))
  end
end
