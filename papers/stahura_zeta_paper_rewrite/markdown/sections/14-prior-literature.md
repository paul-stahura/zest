[← Contents](../README.md) · [← 13 Further observations](13-further-observations.md) · [15 Glossary →](15-glossary.md)
...
### 14.1 Nickel’s Argand-diagram geometry

Nickel \[17\], with a condensed sequel \[18\], starts from the same object this paper starts from, the Argand diagram of the steps $`n^{-s}`$, and reaches a construction close enough to ours that the two are worth setting side by side. They agree in shape, and they differ in the one respect this paper is about.

<a id="dictionary-"></a>

##### Dictionary.

His center of symmetry is the step $`n_p=\bigl\lfloor\sqrt{t/2\pi}\,\bigr\rfloor`$, with fractional part $`p=\sqrt{t/2\pi}-n_p`$. Since $`\sqrt{t/2\pi}\approx T+\tfrac12`$ (§[10.1](10-geometric-origins-of-the-i-t-and-yin-and-yang-fu.md#sec-IT-origin)), his index is $`n_p\approx\lfloor T+\tfrac12\rfloor`$ and $`p\approx\{T+\tfrac12\}`$, so it nearly agrees with our $`m=\lfloor T\rfloor`$ on the first half of each unit interval and exceeds it by one on the second half: the same half-unit offset already recorded for Siegel’s cutoff in §[8.2](08-other-remainders.md#sec-kuznetsov). His factor $`Q(s)`$, defined by $`\zeta(s)=Q(s)\zeta(1-s)`$, is our $`\chi(s)`$; the displayed form $`Q=n_p^{1-2s}e^{i(t+\pi/4)}`$ is the familiar $`e^{-2i\vartheta(t)}`$ asymptotic, which reproduces χ to a relative $`1.5\times10^{-5}`$ at $`T=20.3`$ and $`1.6\times10^{-7}`$ at $`T=200.3`$ once the smooth $`\sqrt{t/2\pi}`$ is used in place of the rounded $`n_p`$. His symmetry angle Θ is our $`\tfrac{\psi}{2}=\tfrac12\arg\chi`$.

<a id="what-matches-"></a>

##### What matches.

Under that dictionary his Riemann–Siegel equation $`\zeta=P(s)+Q(s)P(1-s)`$ is our two-leg decomposition $`\zeta=B_1+B_2`$ with $`B_2=\chi\,\overline{B_1}`$ (Corollary [9.1](09-the-geometry-behind-the-result-experimental-math.md#cor-equal-legs), verified in Lean in Appendix [C](c-lean-formalization-of-the-critical-line-d1-d2-r1.md#app-lean-critical)); his polar form $`\zeta=2\cos(\phi-\Theta)\,\mathsf{P}e^{i\Theta}`$ is the two-leg identity $`\zeta=B_1+\chi\overline{B_1}=2|B_1|\cos(\arg B_1-\tfrac{\psi}{2})e^{i\psi/2}`$, not the polar decomposition of *R*; and his observation that the two magnitudes are equal exactly when $`\sigma=\tfrac12`$ is the on-line half of Corollary [9.1](09-the-geometry-behind-the-result-experimental-math.md#cor-equal-legs) (the present legs can also be equal on isolated off-line loci). He also writes the classical remainder as $`R=2\mathsf{L}\cos(\theta_L-\Theta)`$, where $`L=\mathsf{L}e^{i\theta_L}`$ runs from the joint at step $`n_p`$ to his center: that is exactly the role played here by the pair $`(d_1,-\omega)`$. Where his index agrees with ours the two projected remainders converge, the ratio $`2\mathsf{L}\cos(\theta_L-\Theta)\,/\,2d_1\cos(\omega+\tfrac{\psi}{2})`$ falling through $`2.24`$, $`1.34`$, $`1.11`$, $`1.03`$, $`1.01`$ at $`T=6.3,\,20.3,\,60.3,\,200.3,\,600.3`$.
...
```math
G(s)=\zeta(s)+\frac{\zeta'(s)}{f'(s)+f'(1-s)}\qquad\text{(191)}
```
...
f'(s)+f'(1-s)=-\frac{\chi'}{\chi}(s),\qquad\text{(192)}
```

at every *s* where the displayed quotients are defined. That is the weight of the off-line form [(165)](12-theta1-theta2-and-the-zero-counting-function.md#eq-vel-split-offline), so $`G=B_1^{\ast}`$ as a meromorphic identity wherever those quotients are finite and nonzero. On the critical line the same weight turns real: with $`s=\tfrac12+it`$ we have $`1-s=\bar{s}`$ and $`f'(1-s)=\overline{f'(s)}`$, so the sum is twice a real part,
...
=\vartheta(t)`$. So the angle he watches is the one [(162)](12-theta1-theta2-and-the-zero-counting-function.md#eq-N-star) is built from,
...
One further reading of the same diagram deserves mention. Kapitonets \[12\] draws his axis of symmetry through the *midpoint* of the remainder vector at $`\sigma=\tfrac12`$, observes that the remainder is then perpendicular to that axis, and concludes both that away from zeros, where $`\arg\zeta`$ is defined, $`\arg\zeta`$ is the normal direction, and that the two chains project onto the axis with canceling sums. That is the half-split of §[8.1](08-other-remainders.md#sec-remainders-summary) together with Corollary [9.2](09-the-geometry-behind-the-result-experimental-math.md#cor-bisector-proj), read off the picture. His other construction, recovering ζ by repeatedly replacing a polygon of partial sums by the polygon of their midpoints, is unrelated to the bisector point: iterated midpointing is the binomial transform, and the limit is Cesàro summation of the Dirichlet series, as he says himself.
