...
description: Create an ESLint rule as a behavioral guardrail for Claude. Use when you've noticed Claude repeatedly making the same mistake or drifting from an established pattern - the rule becomes a persistent nudge that keeps Claude aligned without repeated instructions.
allowed-tools: Read, Write, Edit, Grep, Glob, Bash
---

# Creating ESLint Rules as Claude Guardrails

## Philosophy

### Why ESLint rules for Claude?

These rules are **behavioral nudges** - persistent reminders that keep Claude aligned with codebase conventions without you having to re-explain them every session.

When Claude writes code that violates a rule, it sees the lint error immediately. This creates a tight feedback loop: Claude corrects itself in real-time. The alternative - correcting Claude manually each time - doesn't scale and the corrections are lost between sessions.
...
We're not just fixing violations once - we're building **institutional knowledge**. The next time someone (human or Claude) encounters this error, they should have a concrete, battle-tested guide to fixing it.
...
- An architectural pattern exists but Claude doesn't follow it consistently
- A convention is important but easy to forget
- You've corrected Claude for the same thing 3+ times
- **Critically: the violation can be expressed as a reliable code pattern**

**The test**: If it's worth correcting Claude for, it's worth automating that correction.
...
**The litmus test**: Can you describe the violation as "when I see [specific AST pattern] in [specific file context]"? If yes, it's a good candidate. If you need to understand what the code _means_ rather than what it _looks like_, consider other approaches (documentation, CLAUDE.md rules, code review).
...
- **Validation patterns** - using the right validation approach in the right place (see [Validators docs](.claude/rules/validators.md))
...
See the example template at `.claude/skills/add-eslint-rule/example-rule.js` for the full structure.

Create `eslint-local-rules/<rule-name>.js` with:

1. **Header comment** - Why this rule exists, what triggers it, how to fix violations
2. **meta.messages** - Actionable error messages that tell Claude what to do instead
...
This documentation ensures that anyone encountering this error in the future - human or Claude - has a concrete guide to fixing it. We're not just fixing violations once; we're building institutional knowledge.
...
- Example template: `.claude/skills/add-eslint-rule/example-rule.js`
- ESLint custom rules: https://eslint.org/docs/latest/extend/custom-rules
- Validators guidance: `.claude/rules/validators.md`
